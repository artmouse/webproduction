...
class Action_Status_CronMinute implements Events_IEventObserver {
    ...
    public function notify(Events_Event $event) {
        ...
        while ($x = $blocks->getNext()) {
            ...
            if (!method_exists($block, 'processCronMinute')) {
                continue;
            }
            ...
            ModeService::Get()->verbose(
                'Action_Status_CronMinute statusID='.$x->getStatusid().' contentID='.$x->getContentid()
            );

            $block->setValue('data', $x->getData());
            $block->setValue('status', $status);
            $block->processCronMinute($event);
        }
    }
