...
class shop_product extends Engine_Class {
    ...
    public function process() {
        ...
        foreach ($product->getImagesArray() as $image) {
            ...
            $imageSize = Engine::Get()->getConfigFieldSecure('product-image-size');
            $width_image = 330;
            $height_image = 225;
            if ($imageSize) {
            ...
            if ($product->getImagecrop() && !$cropImageAdd) {
                $cropUrl = Shop_ImageProcessor::MakeThumbProportional(
                    PackageLoader::Get()->
                    getProjectPath().'/media/shop/'.$product->getImagecrop(), $width_image, $height_image, $format
                );
                $cropImageAdd = true;
            } else {
                $cropUrl = Shop_ImageProcessor::MakeThumbProportional(
        ...
        $this->setValue('commentsArray', $this->getValue('commentsArray'));
