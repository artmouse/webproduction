...
class Datasource_News extends Forms_ADataSourceSQLObject {
    ...
    protected function _initialize() {
        ...
        $field->setName(Shop_TranslateFormService::Get()->getTranslateSecure('translate_title'));
        ...
        $field->setName(Shop_TranslateFormService::Get()->getTranslateSecure('translate_short_contents'));
        $this->addField($field);

        $field = new Forms_ContentFieldCKEditor('content');
        $field->setName(Shop_TranslateFormService::Get()->getTranslateSecure('translate_contents'));
        $this->addField($field);

        $field = new Forms_ContentFieldSelectList('pageid', true);
        $field->setName('Страница');
        $field->setDataSource(new Datasource_TextPages(false));
        $this->addField($field);
        ...
        $field->setName(Shop_TranslateFormService::Get()->getTranslateSecure('translate_article_about_this_product'));
        $field->setDataSource(new Datasource_Products());
        $this->addField($field);

        $field = new Forms_ContentFieldSelectList('categoryid', true);
        $field->setName(Shop_TranslateFormService::Get()->getTranslateSecure('translate_article_about_the_category'));
        $field->setDataSource(new Datasource_Category());
        $this->addField($field);

        $field = new Forms_ContentFieldSelectList('brandid', true);
        $field->setName(Shop_TranslateFormService::Get()->getTranslateSecure('translate_article_about_the_brand'));
        ...
        try {
            ...
            try {
                ...
                CommentsAPI::Get()->addComment(
                    'shop-history-news-'.$r,
                    Shop_TranslateFormService::Get()->getTranslateSecure('translate_added_news').' #'.$r.' '.$x->getName(),
                    $user->getId()
                );
            } catch (Exception $e) {
        ...
        try {
            ...
            try {
                ...
                CommentsAPI::Get()->addComment(
                    ...
                    Shop_TranslateFormService::Get()->getTranslateSecure('translate_edited_news').' #'.$news->getId().' '.$news->getName()."\n".implode("\n", $diffArray),
                    $user->getId()
                );
            } catch (Exception $e) {
    ...
    public function delete($key) {
        try {
            ...
            try {
                ...
                CommentsAPI::Get()->addComment(
                    'shop-history-news-'.$news->getId(),
                    Shop_TranslateFormService::Get()->getTranslateSecure('translate_remove_news').' #'.$news->getId().' '.$news->getName(),
                    $user->getId()
                );
            } catch (Exception $e) {
