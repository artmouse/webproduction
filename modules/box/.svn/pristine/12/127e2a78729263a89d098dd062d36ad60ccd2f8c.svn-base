<?php
class box_block_graph_load extends Engine_Class {
    ...
    public function process() {
        ...
        foreach ($issueCountArray as $managerID => $issueCount) {
            ...
            try {
                ...
                $b['avatar'] = $user->makeImageThumb(200);
    ...
    private function _makeIssueCountArrayByManager($parentID, $resultArray) {
        ...
        while ($sub = $subIssues->getNext()) {
            if ($sub->getId() != $parentID) {
                ...
                $resultArray = $this->_makeIssueCountArrayByManager($sub->getId(), $resultArray);
            }
        }
