 ...
 * @package OneBox
 */
class BoxNotify_UnknownEmailFull {
    ...
    public function process() {
        ...
        $q = $connection->query("SELECT `to`, `from`, COUNT(*) AS cnt FROM shopevent WHERE `type`='{$type}' AND cdate >= NOW() - INTERVAL 1 MONTH GROUP BY `to`");
        while ($x = $connection->fetch($q)) {
            ...
            try {
                ...
                $content .= ', Вы написали письма ('.$cnt.' шт.) на ранее неизвестный email '.$to.'. Необходимо создать новый контакт с таким email или добавить контакт в существующую карточку контакта.';
                $content .= "\n";
