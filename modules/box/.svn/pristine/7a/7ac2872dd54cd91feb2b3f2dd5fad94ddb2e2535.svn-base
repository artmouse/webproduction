<?php
class box_block_product_list extends Engine_Class {
    ...
    public function process() {
        ...
        while ($x = $orderproducts->getNext()) {
                ...
                while ($storage = $storages->getNext()) {
                    ...
                    if ($balance) {
                        try {
                            ...
                            if (!$reserveOK) {
                                $reserveOK = (@$storageArray[$storage->getId()]['linked'] >= $x->getProductcount());
                            }
            ...
            $productName = htmlspecialchars($x->getProductname());
            $productNameData = $productName;
            
            if (strpos($order->getLinkkey(), 'autoOrderSupplier-') === 0) {

                try {
                    $supplierCode = '';

                    $orProduct = Shop::Get()->getShopService()->getProductByID($x->getProductid());

                    $supplierId = str_replace('autoOrderSupplier-', '', $order->getLinkkey());

                    if ($supplierId) {
                        $tmpProductSupplier = new XShopProductSupplier();
                        $tmpProductSupplier->setSupplierid($supplierId);
                        $tmpProductSupplier->setProductid($orProduct->getId());
                        $tmpProductSupplier->addWhere('code', '', '<>');
                        $tmpProductSupplier = $tmpProductSupplier->getNext();
                        if ($tmpProductSupplier) {
                            $supplierCode = $tmpProductSupplier->getCode();
                        }
                    }
                    
                    $productArticul = $orProduct->getArticul();

                    if ($productArticul) {
                        $productNameData = 'Арт. '.$productArticul.' '.$productName;
                    }

                    if ($supplierCode) {
                        $productNameData = 'Пост. '.$supplierCode. ' '.$productName;
                    }
        ...
        try{
            $this->setValue('isOutcoming', !$order->getWorkflow()->getOutcoming());
        } catch (Exception $eworkflow) {
            $this->setValue('isOutcoming', !$order->getOutcoming());
        }

        try {
            if (Shop_ModuleLoader::Get()->isImported('storage') && $order->getWorkflow()->getOutcoming()) {
                // склады
