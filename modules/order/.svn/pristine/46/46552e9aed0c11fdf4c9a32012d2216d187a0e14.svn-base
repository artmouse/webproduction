<?php
class order_list extends Engine_Class {
        ...
        if ($this->_datasource === null) {
            ...
            if (!$this->_datasource) {
                if (!Shop_ModuleLoader::Get()->isModuleInModulesArray('box') 
                    ...
                    $this->_datasource = new Datasource_Orders();
                } else {
                    $this->_datasource = new Datasource_IssueCustom();
                }
    ...
    public function process() {
        ...
        // получаем режим, в котором будем показывать список
        ...
        foreach ($clientIDArray as $clientID) {
            try {
                ...
                'text' =>  $client->getTypesex() ==
                    'company' ? Shop::Get()->getTranslateService()->getTranslateSecure('translate_kompaniya_').
