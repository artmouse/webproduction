    ...
    public function process() {
        ...
        while ($d->__toString() <= $dateToFormatted) {
            ...
            if ($groupBy == 'day') {
                ...
                $dateStart = DateTime_Object::FromString($tmp[0].'-01-01')->
                    addDay(+$tmp[1]*7-7)->setFormat('Y-m-d')->__toString();
                $dateEnd = DateTime_Object::FromString($tmp[0].'-01-01')->
                    addDay(+$tmp[1]*7)->setFormat('Y-m-d')->__toString();
