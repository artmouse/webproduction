...
class order_add extends Engine_Class {
    ...
    public function process() {
        ...
        try {
                    ...
                    foreach ($this->getArguments() as $key => $item) {
                        if (preg_match('#add_product_([\d]+)#uisD', $key, $r)) {
                            ...
                            $productArray[] = array(
                                ...
                                'linkkey' => $this->getArgumentSecure('linkkey_product_'.$count)
                            );
                        }
                    }
            ...
            if ($this->getArgumentSecure('creatybyprojectid')) {
                $project = Shop::Get()->getShopService()->getOrderByID($this->getArgumentSecure('creatybyprojectid'));
                $this->setValue('clientname', $project->getClientname());
                $this->setValue('contact_phone', $project->getClientphone());
                $this->setValue('contact_email', $project->getClientemail());
            }
            ...
            if ($this->getArgumentSecure('creatybyprojectid')) {
                $this->setControlValue('parentid', $this->getArgumentSecure('creatybyprojectid'));
            }
