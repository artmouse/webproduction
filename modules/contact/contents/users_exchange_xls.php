...
class users_exchange_xls extends Engine_Class {
    ...
    public function process() {
        ...
        $emailToArray = $this->getUser()->getEmail();
        if ($emailToArray) {
            $emailToArray = array($emailToArray);
        }
        if (!$emailToArray) {
    ...
    private function _xlsExport($groupID = '', $emails = '') {
        try {
    ...
    private function _makeGroupArray() {
        $a = array();
        $groups = Shop::Get()->getUserService()->makeUserGroupTree();
        foreach ($groups as $x) {
            $a[] = array(
            'id' => $x->getId(),
            'name' => $x->makeName(),
