<?php
class storage_passport_target_ajax_update extends Engine_Class {
        ...
        try {
            ...
            if ($this->getControlValue('saveok')) {
                try {
                    ...
                    $productItem = new XShopProductPassportItem();
                    $productItem->setIstarget(1);
                    $productItem->setProductid($this->getArgument('productid'));
                    $productItem->setPassportid($passport->getId());
                    $productItem = $productItem->getNext();
                    ...
                    if (!$productItem) {
                        ...
                        $productItem->setProductid($this->getArgument('productid'));
                        $productItem->setPassportid($passport->getId());
                        $productItem->setAmount($this->getArgument('count'));
                        $productItem->insert();
                    } else {
                        $productItem->setAmount($this->getArgument('count'));
                        $productItem->update();
