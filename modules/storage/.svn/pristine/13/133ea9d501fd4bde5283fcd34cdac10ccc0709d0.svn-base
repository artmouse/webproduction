            ...
            try {
                ...
                while ($x = $balance->getNext()) {
                    try {
                        ...
                        'cost' => $x->getCost(),
                        'serial' => $x->getSerial(),
                        'productURL' => Engine::GetLinkMaker()->makeURLByContentIDParam(
                            'shop-admin-products-edit', $x->getProductid()
                        ),
                        'reserveURL' => Engine::GetLinkMaker()->makeURLByContentIDParam(
                            'shop-admin-storage-reserve', $x->getId(), 'balanceid'
                        ),
                        'historyURL' => Engine::GetLinkMaker()->makeURLByContentIDParam(
                            'shop-admin-storage-motion-balance-history', $x->getId(), 'balanceid'
                        )
                        );
