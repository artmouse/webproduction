...
class Checker {
    ...
    public static function CheckEmail($email) {
        ...
        if (preg_match('/^([0-9a-z^\-\.\_]+)@(([0-9a-z\-\_]+\.)+)([a-z]{2,})$/i', $email, $r)) {
            ...
            if (preg_match("/([\.]{2,})/i", $name)) return false;
     ...
     * @return bool
     */
    public static function CheckLogin($login, $minLength = 3) {
        if ($minLength <= 0) {
            $minLength = 3;
        }
        return (bool) preg_match('/^[a-z0-9]{'.$minLength.',}$/i', $login);
    }
    ...
    public static function CheckHostname($hostname, $cyrillic = false) {
        ...
        // дополнительно проверяем, чтобы хост не начинался на дефис
        if (preg_match("/^-/i", $hostname)) return false;
     ...
     * @return bool
     */
    public static function CheckImageFormat($filepath, $allowMIME = array('image/gif', 'image/jpeg', 'image/png', 'image/pjpeg')) {
     ...
     * @author Ramm
     * @param float $number
     * @param bool $correct если true, то возвращает числовое значение в отформатированном виде иначе не меняет первое после проверки
     * @return mixed
     */
     ...
     * @return bool
     */
    public function StringInUTF8($string) {
     ...
     * @return bool
     */
    public static function CheckFileFormat($type, $allowMIME = array('application/vnd.ms-excel', 'application/pdf', 'application/zip', 'application/msword')) {
